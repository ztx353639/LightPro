...
class VGG(nn.Module):
    def __init__(self):
        ...
        self.post = post()
        self.Block = [self.block1, self.block2, self.block3, self.block4, self.block5, self.block6]
        self.feature = [self.Block[i][k] for i in range(len(self.Block)) for k in range(len(self.Block[i]))]
...
class SSD300(nn.Module):
    def __init__(self, num_classes, backbone_name, pretrained=False):
        ...
        if backbone_name == "vgg":
            self.vgg = nn.Sequential(*vgg(pretrained).feature)
